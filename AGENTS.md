...
- **10 managed prompts** `mailroom-<agent_name>` (v1, `production` label) synced via `scripts/sync_prompts.py`; generations carry `langfuse_prompt=` so every trace links its prompt version.
...
- **1 project-scope LLM-as-a-Judge evaluator** (`mailroom-pipeline-judge`, numeric 0-1) with **1 observation rule** (`mailroom-pipeline-rule`) matching the single `pipeline-result` generation emitted per document trace — exactly one cumulative judge call per document (classification + correctness + completeness). Synced via `scripts/sync_evaluators.py`, which prunes stale mailroom evaluators/rules; the 22 `managed` template evaluators are platform-locked (403 on delete) — ignore them.
