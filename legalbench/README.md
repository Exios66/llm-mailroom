...
3. **Experiment log** — one JSON record appended to the shared append-only
   ...
   `../llm-entity-extraction` exists, else `legalbench/reports/`). The
...
| Knob | Default |
|---|---|
| `LEGALBENCH_EXPERIMENT_LOG` | sibling `reports/experiment_log.jsonl`, else `legalbench/reports/experiment_log.jsonl` |
...
1. Add a loader to `legalbench/data.py` (corpus → sampled rows with
   `expected` labels).
2. Add a versioned prompt to `legalbench/prompts.py` (version = experiment
   identity; never mutate after a logged run).
3. Register the task in `legalbench/tasks.py` (loader, call shape, extractor,
   scorer, classes).
...
- Corpora are the full local CUAD mirror; re-run `scripts/fetch_full_cuad.py`
