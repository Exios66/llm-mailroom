...
The same rubrics are configured as **one cumulative live LLM-as-a-Judge evaluator in the Langfuse project** (`scripts/sync_evaluators.py`): the pipeline emits a single `pipeline-result` generation per document trace (`graph/build_graph.py:_emit_pipeline_result`), and one observation rule (`mailroom-pipeline-rule`) matches it — so each document costs exactly **one judge call** returning a **binary CORRECT/MISS verdict** (evaluator `mailroom-pipeline-judge`). When the caller knows the expected outcome, the ground truth (`expected_doc_class`, `expected_stage`, and for grounded runs literal per-field `expected_fields`) is embedded in the generation output and the judge decides **strictly against the actual ground truth**; grounded runs (with `expected_fields`) skip the document text in the judge input — the input is the small extracted-vs-expected payload, cutting ~90% of judge tokens. Live runs without ground truth fall back to rubric judgment. The script also ensures an LLM connection for the judge provider exists (OpenRouter key from `.env`) and prunes any stale mailroom evaluators/rules.
