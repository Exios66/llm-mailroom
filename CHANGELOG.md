...
- **HF ground_truth join.** `run_hf_pilot.py` loads Hub config `ground_truth` fields `expected` (docclass) and `expected_subclass`, joined to `default` `doc_text` on filename. CUAD folder names are never used as classes. Oversized MAUD mergers stay in the stratified pool and are truncated at `--max-chars`.
