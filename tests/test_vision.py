...
def test_render_caps_pages():
    ...
    ten = render_pdf_pages(Path("examples/samples/contract/contract_03_service_agreement.pdf"), cap=10)
    assert len(ten) == 10
    # …while cap=0 (or None) means "ALL pages" — no document content is ever
    # dropped by the page cap.
    all_pages = render_pdf_pages(Path("examples/samples/contract/contract_03_service_agreement.pdf"), cap=0)
